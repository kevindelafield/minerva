...
namespace util
{
    ...
    static void jo_writeBits(std::vector<unsigned char> & output, int &bitBuf, int &bitCnt, const unsigned short *bs) {
        bitCnt += bs[1];
    ...
    static int jo_processDU(std::vector<unsigned char> & output, int &bitBuf, int &bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2]) {
        ...
        int diff = DU[0] - DC; 
        if (diff == 0) {
            jo_writeBits(output, bitBuf, bitCnt, HTDC[0]);
        } else {
            unsigned short bits[2];
            jo_calcBits(diff, bits);
            jo_writeBits(output, bitBuf, bitCnt, HTDC[bits[1]]);
            jo_writeBits(output, bitBuf, bitCnt, bits);
        }
        // Encode ACs
        int end0pos = 63;
        ...
        if(end0pos == 0) {
            jo_writeBits(output, bitBuf, bitCnt, EOB);
            return DU[0];
        }
        for(int i = 1; i <= end0pos; ++i) {
            ...
            if ( nrzeroes >= 16 ) {
                ...
                for (int nrmarker=1; nrmarker <= lng; ++nrmarker)
                    jo_writeBits(output, bitBuf, bitCnt, M16zeroes);
                nrzeroes &= 15;
            }
            unsigned short bits[2];
            jo_calcBits(DU[i], bits);
            jo_writeBits(output, bitBuf, bitCnt, HTAC[(nrzeroes<<4)+bits[1]]);
            jo_writeBits(output, bitBuf, bitCnt, bits);
        }
        if(end0pos != 63) {
            jo_writeBits(output, bitBuf, bitCnt, EOB);
    ...
    bool jo_write_jpg(std::vector<unsigned char> & output, const void *data, int width, int height, int comp, int quality) {
        ...
        output.insert(output.end(), head0, head0 + sizeof(head0));
        output.insert(output.end(), YTable, YTable + sizeof(YTable));
        output.push_back(1);
        output.insert(output.end(), UVTable, UVTable + sizeof(UVTable));
        ...
        output.insert(output.end(), head1, head1 + sizeof(head1));
        output.insert(output.end(), 
                      std_dc_luminance_nrcodes + 1, 
                      std_dc_luminance_nrcodes + 1 + sizeof(std_dc_luminance_nrcodes) - 1);
        output.insert(output.end(),
                      std_dc_luminance_values,
                      std_dc_luminance_values + sizeof(std_dc_luminance_values));
        output.push_back(0x10);
        output.insert(output.end(), 
                      std_ac_luminance_nrcodes + 1, 
                      std_ac_luminance_nrcodes + 1 + sizeof(std_ac_luminance_nrcodes) - 1);
        output.insert(output.end(),
                      std_ac_luminance_values,
                      std_ac_luminance_values + sizeof(std_ac_luminance_values));
        output.push_back(1);
        output.insert(output.end(), 
                      std_dc_chrominance_nrcodes + 1, 
                      std_dc_chrominance_nrcodes + 1 + sizeof(std_dc_chrominance_nrcodes) - 1);
        output.insert(output.end(),
                      std_dc_chrominance_values,
                      std_dc_chrominance_values + sizeof(std_dc_chrominance_values));
        output.push_back(0x11);
        output.insert(output.end(), 
                      std_ac_chrominance_nrcodes + 1, 
                      std_ac_chrominance_nrcodes + 1 + sizeof(std_ac_chrominance_nrcodes) - 1);
        output.insert(output.end(),
                      std_ac_chrominance_values,
                      std_ac_chrominance_values + sizeof(std_ac_chrominance_values));
        ...
        for(int y = 0; y < height; y += 8) {
            for(int x = 0; x < width; x += 8) {
                ...
                DCY = jo_processDU(output, bitBuf, bitCnt, YDU, fdtbl_Y, DCY, YDC_HT, YAC_HT);
                DCU = jo_processDU(output, bitBuf, bitCnt, UDU, fdtbl_UV, DCU, UVDC_HT, UVAC_HT);
                DCV = jo_processDU(output, bitBuf, bitCnt, VDU, fdtbl_UV, DCV, UVDC_HT, UVAC_HT);
            }
        }
	
        // Do the bit alignment of the EOI marker
        static const unsigned short fillBits[] = {0x7F, 7};
        jo_writeBits(output, bitBuf, bitCnt, fillBits);
